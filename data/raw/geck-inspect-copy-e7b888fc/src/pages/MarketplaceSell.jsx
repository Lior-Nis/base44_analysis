...
import { syncWithMorphMarket } from '@/api/functions';
import { syncWithPalmStreet } from '@/api/functions';
...
export default function MarketplaceSellPage() {
    ...
    const handleUpdateUser = async (field, value) => {
        if (!user) return;
        try {
            const updatedUser = await User.updateMyUserData({ [field]: value });
            setUser(updatedUser);
        } catch (error) {
    ...
    const updateGeckoStatus = async (geckoId, newStatus) => {
        try {
            const updatedGecko = await Gecko.update(geckoId, { status: newStatus });
